    ...
    *arr = malloc(sizeof(struct cluster_t)*count);                         //alokacia pre vstupny subor
    ...
    for (int i = 0; i<count; i++){
        ...
        if (arr_obj.id < 0 || arr_obj.x < 0 || arr_obj.y < 0){             //ak je id/suradnica mensia ako 0
            ...
            return -1;                                                     //vrati exit_failure
        }
        if (arr_obj.id > 100 || arr_obj.x > 1000 || arr_obj.y > 1000){     //ak nacitanie id/suradnica vacsia ako v zadani
            fprintf(stderr, "ID musi byt mensie ako 100 a suradnice musia byt mensie ako 1000\n");
            return -1;                                                     //vrati exit_failure
        }

...
{
    // TODO
    free(c->obj);                                                      //uvolni naalokovanu pamat pre objekty v clustri
    c->obj = NULL;                                                     //inicializacia pre prazdny shluk
    c->size = 0;                                                       //nastavi realnu velkost zhluku na 0
    c->capacity = 0;                                                   //celkovu kapacitu zhluku nastavni na 0
}
...
void merge_clusters(struct cluster_t *c1, struct cluster_t *c2)
{
    ...
    // TODO
    int velk2 = c2->size;                                                  //vytvori premennu pre velkost c2 [for]
    if(c1->capacity <= c2->size) {                                         //skontroluje kapacitu clustru
        ...
        for (int i = 0; i < velk2; i++) {                                  //kym je cluster mensi ako c2->size
...
float obj_distance(struct obj_t *o1, struct obj_t *o2)
{
    ...
    float rozdiel_x = (o1->x - o2->x);                                     //rozdiel xovych suradnic
    float rozdiel_y = (o1->y - o2->y);                                     //rozdiel ynovych suradnic
    vzdialenost_obj = sqrtf(powf(rozdiel_x,2) + powf(rozdiel_y,2));        //vzorec pre euklidovsku vzdialenost
...
float cluster_distance(struct cluster_t *c1, struct cluster_t *c2)
{
    ...
    // TODO
    int velk1 = c1->size;                                                  //vytvori premennu pre velkost c1 [for]
    int velk2 = c2->size;                                                  //vytvori premennu pre velkost c2 [for]
    float stara_vzdialenost = INT_MAX;                                     //prvotna vzdialenost - bude prepisana
    float nova_vzdialenost;                                                //pomocna vzdialenost
    for (int i = 0; i < velk1; i++){                                       //porovnava vsetky objekty 0-0/1/2
        for (int j = 0; j < velk2; j++){                                   //potom porovna 1-0/1/2
...
int load_clusters(char *filename, struct cluster_t **arr)
{
    ...
    if (subor == NULL){                                                    //ak sa nepodari otvorit
        ...
        return EXIT_FAILURE;                                               //exit_failure                                                    //vrati neuspesne ukoncenie programu
    }
    ...
    if (fscanf(subor, "count=%d", &count) != 1){                           //ak prvy riadok nebude count=N
        ...
        return EXIT_FAILURE;                                               //exit_failure                                                        //chybne ukoncenie suboru
    ...
    *arr = malloc(sizeof(struct cluster_t)*count);                         //alokacia pre vstupny subor
    ...
    for (int i = 0; i<count; i++){
        ...
        if (arr_obj.id < 0 || arr_obj.x < 0 || arr_obj.y < 0){             //ak je id/suradnica mensia ako 0
            fprintf(stderr, "ID aj suradnice musia byt vacsie ako 0\n");   //chybovy vystup
            init_cluster(&(*arr)[i],CLUSTER_CHUNK);                        //inicializacia kapacity pre shluk v maine bude dealokovany
            append_cluster(&(*arr)[i],arr_obj);                            //prida shluk ale nasledne bude v maine dealokovany
            fclose(subor);
            return -nacitane;                                              //vrati -nacitane kvoli podmienke do mainu
        }
        if (arr_obj.x > 1000 || arr_obj.y > 1000){                         //ak nacitanie id/suradnica vacsia ako v zadani
            fprintf(stderr, "Suradnice musia byt mensie ako 1000\n");
            init_cluster(&(*arr)[i],CLUSTER_CHUNK);                        //inicializacia kapacity pre shluk v maine bude dealokovany
            append_cluster(&(*arr)[i],arr_obj);                            //prida shluk ale nasledne bude v maine dealokovany
            fclose(subor);
            return -nacitane;                                              //vrati -nacitane kvoli podmienke do mainu
...
int main(int argc, char *argv[])
{
    ...
    if (argc == 3) {                                                       //ak je zadany aj argument argv2
        ...
        if (count < 0 || count < arg_n){
            count = abs(count);                                            //vrati absolutnu hodnotu countu, aby sa mohol vykonat cyklus
            ...
            return EXIT_FAILURE;                                           //vrati chybovy vystup
        }
        else {
